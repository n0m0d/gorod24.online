...
$(function(){
	...
	$('#save').click(function(event){
		...
		send.main_cost = $("#main-cost").val();
		send.main_discount = $("#main-discount").val();
		send.main_photo = $("#main-photo").val();
		send.main_group = $("#main-group").val();
		...
		xhr = $.ajax({
			url : "/admin/ajax/",
			type : "post",
			data : { "ajax_action" : "update_product", "send" : send},
			...
			success: function(data){
				...
				history.replaceState({foo: 'bar'}, '', '/admin/products/?id='+data['prod_id']);
				$('#main-id').val(data['prod_id']);
				$('#main-id-d').val(data['prod_id']);
				$('#main-createdate').text(data['product']['prod_createdate']);
				$('#post_status-text').text( (data['product']['prod_status'] == 1)? 'Активен': 'Неактивен' );
				}
			},
...
<div id="wrap-main">
	...
	<tr>
		...
		<input type="hidden" value="<?php echo (!is_array($main) and $main=='new')? 'new' :$main['prod_id']; ?>" id="main-id" />
			
			<div class="margintop30">
				<label><h3>ID</h3></label>
				<div class="border" style="text-align:center;"><input style="width:300px;text-align:center;" disabled="disabled" id="main-id-d" type="text" value="<?php echo ((!is_array($main) and $main=='new')? '' : $main['prod_id']); ?>"></div>		
			</div>
			
			<div class="margintop30">
				<label><h3>Название</h3></label>
				<div class="border"><input id="main-name" type="text" value="<?php echo ((!is_array($main) and $main=='new')? '' : $main['prod_name']); ?>"></div>		
			</div>
			
			<div class="margintop30">
				<label><h3>Стоимость (Пример: 000.00)</h3></label>
				<div class="border"><input id="main-cost" type="text" value="<?php echo ((!is_array($main) and $main=='new')? '' : $main['prod_cost']); ?>"></div>		
			</div>
			
			<div class="margintop30">
				<label><h3>Скидка (Пример: 000.00)</h3></label>
				<div class="border"><input id="main-discount" type="text" value="<?php echo ((!is_array($main) and $main=='new')? '' : $main['prod_discount']); ?>"></div>		
			</div>
			
			<div class="margintop30">
				<label><h3>Фото (перечислить через запятую URL картинок)</h3></label>
				<div class="border"><input id="main-photo" type="text" value="<?php echo ((!is_array($main) and $main=='new')? '' : $main['prod_photo']); ?>"></div>		
			</div>
			...
			<div class="margintop30">
				...
				<div class="tabs-body">
					<?php 
					echo apply_filters('the_editor_content', ((!is_array($main) and $main=='new')? '' : $main['prod_description']));
					?>
				</div>
			</div>
		...
		<td style="width:260px;">
			<div class="right-block" id="page-controls">
				...
				<div class="controls-body">
					<div class="controls-body-row">Статус: <strong id="post_status-text"><?= (($main['prod_status'] == 1)? "Активен " : "Неактивен"); ?></strong><div class="switch demo3"><input id="main-status" type="checkbox" <?= (($main['prod_status'] == 1)? "checked='checked'" : ""); ?>><label><i></i></label></div> </div>
					<div class="controls-body-row">Дата создания: <strong id="main-createdate"><?= ((!is_array($main) and $main=='new')? '' :$main['prod_createdate']); ?></strong></div>
			...
			<div class="right-block" id="page-controls">
				<div class="controls-header">Группа:</div>
				<div class="controls-body">
					<div class="controls-body-row"><select id="main-group" class="block-select">
					<option value="0">Выберите группу</option>
					<?php
					foreach ($qroups as $i => $g){
						$s = ($g['group_id']== $main['group_id'])?'selected="selected"':'';
