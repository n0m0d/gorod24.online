...
$(function(){
	...
	$('#delete-items').click(function(){
		...
		xhr = $.ajax({
			url : "/admin/ajax",
			type : "post",
			data : { "ajax_action" : "delete_products", "items" : items},
...
<div id="icon-edit-pages" class="icon32 icon32-posts-page"><br></div><h2>Товары и услуги</h2>
<div id="wrap-main">
	<div>
		<h3>Основные действия над товарами и услугами (т/у)</h3>
		<a href="/admin/products/?id=new" class="button-controls add">Добавить т/у</a>
		<button id="delete-items" class="button-controls delete">Удалить отмеченные т/у</button>
	</div>
	<div class="page-view">
	...
	<table class="main-table" >
		...
		<tbody>
			...
			<?php 
				...
				{
					$alt = ($i%2 ==0)? "alt":"";
					switch($item['prod_status']){
						case 0 : $status_t = "Неактивен"; break;
						case 1 : $status_t = "Активен"; break;
					}
					
		echo "
			<tr id='raw_{$item['prod_id']}'>
				<td class=\"{$alt} all-align-center\"><input class=\"ch\" type=\"checkbox\" value=\"{$item['prod_id']}\" /></td>
				<td class=\"{$alt}\">{$item['prod_id']}</td>
				<td class=\"{$alt}\"><a href=\"/admin/products/?id={$item['prod_id']}\">"._(apply_filters('name-ru', $item['prod_name']))."</a></td>
				<td class=\"{$alt}\"><a href=\"/admin/productsgroups/?id={$item['group_id']}\">"._(apply_filters('name-ru', $item['group_name']))."</a></td>
				<td class=\"{$alt}\">{$item['prod_createdate']}</td>
