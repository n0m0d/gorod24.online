...
class controller_api extends Controller
{
	...
	function action_adv_pay($params = array()){
		...
		if($method!='PersonalAccount'){
			...
			if($answer_data['target'] and $answer_data['fields']){
				...
				foreach($answer_data['fields'] as $key=>$val){ $l[]="{$key}=".urlencode(strip_tags($val)); }
	...
	function action_adv_up($params = array()){
		...
		$adv_id = (int)addslashes(urldecode($params[2]));
		$access_token=$this->request['access_token'];
		$force = ($this->request['force']?true:false);
		...
		$result = $this->_model_adv->AdvToUp($adv_id, 60*60*24*7, $force);
		echo self::getResponse($result);
	}
	...
	public function action_test_push_android($params = array()){
		$city_id = (int)addslashes(urldecode($params[0]));
		$user_id = (int)addslashes(urldecode($params[1]));
			$topic = '/topics/user-'.$user_id;
			$links = [
				"Добавление новости (тест)" => 'city24:news.add',
				"Мои новости (тест)" => 'city24:news.mylist',
				"Избранные новости (тест)" => 'city24:news.favourites',
				"Все Новости (тест)" => 'city24:news.razd/0',
				"Добавление объявы (тест)" => 'city24:adv.add',
				"Мои объявы (тест)" => 'city24:adv.mylist',
				"Избранные объявы (тест)" => 'city24:adv.favourites',
				"Работа объявы (тест)" => 'city24:adv.razd/13/0 ',
			];
			$i = 0;
			...
			$data = array(
				...
				'data' => [
					"title" => "Город24",
					"message" => $title,
					"id" => (int)(time() + rand(1,1000000) + $i++),
					"link" => $link,
					"image" => 'https://gorod24.online/thrumbs/news/new_64859_361_240.jpg'
				]
			);
			$options = array(
				'http' => array(
	...
	public function action_test_push_ios($params = array()){
		$city_id = (int)addslashes(urldecode($params[0]));
		$user_id = (int)addslashes(urldecode($params[1]));
		
		$devices = $this->_accounts->devices()->getItemsWhere("`uid`='{$user_id}' AND LOWER(`os`)='ios'");
		$certPath = APPDIR . '/Apple_Push_Services_com.mobilemedia.city24.pem';
		$certPath = APPDIR . '/City24iOSPushProduction.pem';
		$certPass = '123123';
		...
		$certPath = APPDIR . '/uploads/application/City24Push.pem';
		$certPass = "city24p543";
		//$RootCertificat = APPDIR . '/uploads/application/City24PrivateKey(1).pem';
		
		
		require_once APPDIR . '/application/core/ApnsPHP/Autoload.php';
		$push = new ApnsPHP_Push(ApnsPHP_Abstract::ENVIRONMENT_PRODUCTION, $certPath);
		if($certPass) { $push->setProviderCertificatePassphrase($certPass); }
		if($RootCertificat) { $push->setRootCertificationAuthority($RootCertificat); }
		$message = new ApnsPHP_Message();
		$listTokens = array();
		foreach ($devices as $device) {
			$message->addRecipient($device['push_token']);
			$listTokens[] = $device['push_token'];
		}
		$push->connect();
		if($decoded){
			$decoded['badge'] = (int)$decoded['badge'];
			if($decoded['badge']) $message->setBadge($decoded['badge']);
			
			$message->setText($decoded['text']);
			if($decoded['sound'] AND $decoded['sound']!='default'){
				$message->setSound($decoded['sound']);
			} else { $message->setSound(); } 
			
			if($decoded['expiry']){
				$message->setExpiry($decoded['expiry']);
			} 
			
			if($decoded['CustomPropertys']){
				foreach($decoded['CustomPropertys'] as $name =>$CustomProperty){
					$message->setCustomProperty($CustomPnameroperty, $CustomProperty);
				}
			} 
		}
		else {
						...
						foreach ($error['ERRORS'] as $m) {
							if (isset($m['statusMessage']) && $m['statusMessage'] == 'Invalid token') {
								...
								if (isset($listTokens[$arrayID])) {
									...
									echo "Удален ошибочный токен! --- " . $listTokens[$arrayID];
...
END:VCARD\r\n", FILE_APPEND);
	...
	/* API method menu.get
	 * Возвращает меню для города и пользователя
	 * https://gorod24.online/api/menu.get/<!city_id!>/<!user_id!>?publickey=<!YOUR_PUBLIC_KEY!>&access_token=<!access_token!>
	 */
	function action_menu_get($params = array()){
		self::log('menu.get', $params);
		$city_id = (int)addslashes(urldecode($params[0]));
		$user_id = (int)addslashes(urldecode($params[1]));
		$access_token=$this->request['access_token'];
		if(
		...
		$result = [ 
			"width" => "85%",
			"top" => [
				[ "type" => "circle", "status" => "locked",   "link" => "city24:messages", "title" => "Ваши сообщения", "counter" => 5, "icon" => "https://gorod24.online/uploads/appmenuicons/01message.png" ],
				[ "type" => "circle", "status" => "unlocked", "link" => "city24:invite", "title" => "Пригласить друга", "counter" => 1, "icon" => "https://gorod24.online/uploads/appmenuicons/02add_user.png" ],
				[ "type" => "circle", "status" => "unlocked", "link" => "city24:qr", "title" => "Сканер QR кода", "counter" => null, "icon" => "https://gorod24.online/uploads/appmenuicons/03qr.png"  ],
				[ "type" => "circle", "status" => "unlocked", "link" => "city24:info", "title" => "Информация", "counter" => null, "icon" => "https://gorod24.online/uploads/appmenuicons/04info.png"  ],
				[ "type" => "circle", "status" => "unlocked", "link" => "city24:settings", "title" => "Настройки", "counter" => null, "icon" => "https://gorod24.online/uploads/appmenuicons/05option.png"  ],
			],
			"main" => [
				[ 
					"type" => "group",
					...
					"items" => [
						[ 
							...
							"counter" => 2,
							"status" => "unlocked",
							"icon" => "https://gorod24.online/uploads/appmenuicons/06news.png",
							"title" => "Новости",
							"link" => "city24:news.razd/0",
							"buttons" => [
								[ "icon" => "https://gorod24.online/uploads/appmenuicons/00settings.png", "link" => "city24:menu.settings/news"  ]
							],
							"items" => [
								[ "title" => "Происшествия", "link" => "city24:news.razd/6" ],
								[ "title" => "Новости", "link" => "city24:news.razd/7" ],
								[ "title" => "Потребителю", "link" => "city24:news.razd/12" ],
							]
						],
						[ 
							"type" => "item",
							"counter" => null,
							"status" => "unlocked",
							"icon" => "https://gorod24.online/uploads/appmenuicons/07ob.png",
							"title" => "Объявления",
							"link" => "city24:adv",
							"buttons" => [
								[ "icon" => "https://gorod24.online/uploads/appmenuicons/00settings.png", "link" => "city24:menu.settings/adv"  ]
							],
							"items" => [
								[ "title" => "Работа", "link" => "city24:adv.razd/13/0" ],
								[ "title" => "Недвижимость", "link" => "city24:adv.razd/16/0" ],
							]
						],
						[ 
							"type" => "item",
							"counter" => null,
							"status" => "unlocked",
							"icon" => "https://gorod24.online/uploads/appmenuicons/08firmi.png",
							"title" => "Фирмы и организации",
							"link" => "city24:biznes",
							"buttons" => [
								[ "icon" => "https://gorod24.online/uploads/appmenuicons/00settings.png", "link" => "city24:menu.settings/biznes"  ]
							],
							"items" => [
								[ "title" => "Авто", "link" => "city24:biznes.razd/69" ],
								[ "title" => "Бизнес", "link" => "city24:biznes.razd/70" ],
							]
						],
					]
				],
				[ 
					"type" => "group",
					"title" => "Народный бренд",
					"items" => [
						[ 
							"type" => "item",
							"counter" => null,
							"status" => "unlocked",
							"icon" => "https://gorod24.online/uploads/appmenuicons/09golosovat.png",
							"title" => "Голосовать",
							"link" => "city24:nb",
							"buttons" => [
								[ "icon" => "https://gorod24.online/uploads/appmenuicons/03qr.png", "link" => "city24:nb.qr"  ]
							],
						],
						[ 
							"type" => "item",
							"counter" => null,
							"status" => "unlocked",
							"icon" => "https://gorod24.online/uploads/appmenuicons/10rezultati.png",
							"title" => "Результаты",
							"link" => "city24:nb.result/11",
						],
					]
				],
				[ 
					...
					"items" => [
						[ 
							...
							"counter" => null,
							"status" => "unlocked",
							"icon" => "https://gorod24.online/uploads/appmenuicons/15radio.png",
							"title" => "Полезное радио",
							"link" => "city24:radio",
						],
						[ 
							"type" => "item",
							"counter" => null,
							"status" => "locked",
							"icon" => "https://gorod24.online/uploads/appmenuicons/16afisha.png",
							"title" => "Афиша",
							"link" => null,
						],
						[ 
							...
							"counter" => null,
							"status" => "locked",
							"icon" => "https://gorod24.online/uploads/appmenuicons/17foto.png",
							"title" => "Фотоконкурс",
							"link" => null,
						],
						[ 
							...
							"counter" => null,
							"status" => "locked",
							"icon" => "https://gorod24.online/uploads/appmenuicons/18rozigrishi.png",
							"title" => "Розыгрыши",
							"link" => null,
						],
					]
