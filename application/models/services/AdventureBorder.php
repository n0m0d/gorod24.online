...
/*
 * Сделать рамку для объявления
 */
class Payment_Services_AdventureBorder extends Payment_Service {
    public function processPay($user_id,$service_id,$item_id,$package_id,$amount, $no_border=null) {
        ...
        if (is_array($adv) and !empty($adv)) {
			...
			if(empty($auto_up)){			
				...
				$adv_model->setRules(
					$adv['id'] /* Id объявления */, 
					date('Y-m-d', $auto_up['start_time']) /* Дата старта поднятия */, 
					(int)($auto_up['days_count'] + ($no_border?0:$package['period_int'])) /* Количество дней */, 
					($auto_up['need_count']+($no_border?0:$c)) /* Автоматических поднятия на срок */, 
					1 /* Включено/выключено */
				);
			}
			$adv_model->AdvToUp($adv['id'], 0, true);
			
            return "Рамка для объявления №{$adv['id']}";
        } else {
            return false;
        }
    }
